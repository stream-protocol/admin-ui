...
const TagInput: React.FC<TagInputProps> = ({
  ...
  return (
    <div className={className}>
      {showLabel && <InputHeader label={label || "Tags (comma separated)"} {...{ required, tooltipContent, tooltip }} className="mb-2" />}
      ...
      >
        <div
          className={clsx(
            "bg-grey-5 shadow-border focus-within:outline-orange-60/10 focus-within:shadow-focus-border rounded-rounded box-border flex h-auto min-h-[40px] items-center px-3 transition-colors focus-within:outline focus-within:outline-4",
            {
          ...
          <div className="h-full">
            <div className="gap-xsmall h-28px box-border flex w-full flex-wrap py-1.5">
              {values.map((v, index) => (
                <div
                  key={index}
                  className={clsx("bg-grey-20 rounded-rounded flex w-max items-center justify-center gap-x-1 whitespace-nowrap px-3 py-1", {
                ...
                >
                  ...
                  <CrossIcon className="text-grey-40 inline cursor-pointer" size="16" onClick={() => handleRemove(index)} />
                </div>
              ))}
              <input
                id="tag-input"
