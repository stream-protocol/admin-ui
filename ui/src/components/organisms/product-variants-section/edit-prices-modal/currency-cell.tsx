 ...
 */
function useCurrencyMeta(currencyCode: string | undefined, regionId: string | undefined) {
...
type CurrencyCellProps = {
  ...
  onDragFillStart: (variantId: string, currencyCode?: string, regionId?: string) => void;

  onMouseCellClick: (event: React.MouseEvent, variantId: string, currencyCode?: string, regionId?: string) => void;

  onInputChange: (value: number | undefined, variantId: string, currencyCode?: string, regionId?: string) => void;
};
...
function CurrencyCell(props: CurrencyCellProps) {
  const { variant, currencyCode, region, editedAmount, isSelected, isAnchor, isInRange, isRangeStart, isRangeEnd, onColumnOver } = props;
  ...
  return (
    <td
      ...
      className={clsx("relative h-10 cursor-pointer border pl-4 pr-2", {
        "bg-orange-100": isSelected && !isAnchor,
      })}
      style={{
        borderTop: isRangeStart && props.isInRangeCol ? "1px double #3B82F6" : "",
        borderBottom: isRangeEnd && props.isInRangeCol ? "1px double #3B82F6" : "",
        borderLeft: props.isRangeStartCol && isInRange ? "1px double #3B82F6" : "",
        borderRight: props.isRangeEndCol && isInRange ? "1px double #3B82F6" : "",
      }}
    >
      <div className="flex">
        ...
        {isRangeEnd && props.isRangeEndCol && !isEditable && (
          <div
            ...
            className="absolute h-2 w-2 cursor-ns-resize rounded-full bg-orange-400"
          />
        )}
      </div>
    </td>
