...
const TimePicker: React.FC<DateTimePickerProps> = ({ date, onSubmitDate, label = "start date", required = false, tooltipContent, tooltip }) => {
  ...
  return (
    <div className="w-full">
      <PopoverPrimitive.Root open={isOpen} onOpenChange={setIsOpen}>
          ...
          >
            <InputContainer className="shadown-none border-0 focus-within:shadow-none">
              ...
              <div className="text-grey-40 flex w-full items-center text-left">
                ...
                <span className="text-grey-90">{date ? moment.utc(date).format("HH:mm") : "--:--"}</span>
              </div>
            </InputContainer>
          </button>
        ...
        >
          <NumberScroller numbers={hourNumbers} selected={selectedHour} onSelect={(n) => setSelectedHour(n as number)} className="pr-4" />
          <NumberScroller numbers={minuteNumbers} selected={selectedMinute} onSelect={(n) => setSelectedMinute(n as number)} />
