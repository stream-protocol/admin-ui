...
function FileSummary(props: FileSummaryProps) {
  ...
  const formattedSize = size / 1024 < 10 ? `${(size / 1024).toFixed(2)} KiB` : `${(size / (1024 * 1024)).toFixed(2)} MiB`;
  ...
  return (
    <div className="relative">
      <Tooltip
        ...
        content={hasError && errorMessage ? <span className="font-normal text-rose-500">{errorMessage}</span> : null}
      >
        <div style={{ width: `${progress}%` }} className="bg-grey-5 transition-width absolute h-full duration-150 ease-in-out" />
  ...
  return (
    <div className="flex gap-6">
      <div className="text-small text-grey-90 flex items-center">
        ...
        <span className="font-semibold"> {creations || 0}&nbsp;</span> {t("upload-modal-new", "new")} {type}
      </div>
      <div className="text-small text-grey-90 flex items-center">
        <WarningCircleIcon fill="#9CA3AF" className="mr-2" />
        <span className="font-semibold">{updates || 0}&nbsp;</span> {t("upload-modal-updates", "updates")}
      </div>
    </div>
  );
}
...
function DropArea(props: DropAreaProps) {
  ...
  return (
    <div
      ...
      className={clsx("mt-3 flex flex-col items-center justify-center rounded-xl border border-dashed p-6", { "opacity-50": isDragOver })}
    >
      ...
      <span className="text-grey-40 text-small">{t("upload-modal-only-csv-files-are-supported", "Only .csv files are supported.")}</span>
    </div>
  );
}
      ...
      <Modal.Body>
        <Modal.Content>
          <div className="flex justify-between">
            <span className="text-grey-90 inter-large-semibold py-4 text-2xl">Import {fileTitle}</span>
          ...
          <div className="text-grey-90 inter-large-semibold mt-8 text-base">{description2Title}</div>
        ...
        <Modal.Footer>
          <div className="flex h-8 w-full justify-end">
            <div className="flex gap-2">
              <Button variant="secondary" className="text-small mr-2 justify-center" size="small" onClick={onClose}>
                {t("upload-modal-cancel", "Cancel")}
              </Button>

              <Button size="small" disabled={!canImport || hasError} variant="primary" className="text-small" onClick={onSubmit}>
