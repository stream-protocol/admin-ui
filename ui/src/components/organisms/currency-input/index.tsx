  ...
  return (
    ...
    >
      <div className={clsx("gap-x-xsmall flex items-center", className)}>
        {!hideCurrency && (
          <div className={clsx({ "w-[144px]": size === "medium" }, { "w-[120px]": size === "small" }, { "flex-1": size === "full" })}>
            {!readOnly ? (
              <Select
                enableSearch
              ...
              <Input label={t("currency-input-currency", "Currency")} value={value?.label} readOnly className="pointer-events-none" tabIndex={-1} />
            )}
          </div>
        )}
...
const Amount = forwardRef<HTMLInputElement, AmountInputProps>(
  (
    ...
    const [formattedValue, setFormattedValue] = useState<string | undefined>(amount ? `${normalizeAmount(currencyInfo?.code!, amount)}` : undefined);
      ...
      <div {...rest}>
        ...
        >
          {currencyInfo?.symbol_native && (
            <Tooltip open={invalid} side={"top"} content={invalidMessage || t("currency-input-amount-is-not-valid", "Amount is not valid")}>
              <span className="inter-base-regular text-grey-40 mr-xsmall">{currencyInfo.symbol_native}</span>
            </Tooltip>
          )}
          <AmountField
            className="remove-number-spinner leading-base text-grey-90 caret-orange-60 placeholder-grey-40 w-full bg-transparent font-normal outline-none outline-0"
