...
const settings: SettingsCardType[] = [
  ...
  },
  {
    ...
    description: "Manage your storefront profile",
    icon: HappyIcon,
  ...
  {
    heading: "The Team",
    description: "Manage users of your storefront Store",
    icon: UsersIcon,
    to: "/a/settings/team",
  },
];

const renderCard = ({ heading, description, icon, to, feature_flag }: SettingsCardType) => {
  const Icon = icon || GearIcon;

  const card = <SettingsCard heading={heading} description={description} icon={<Icon />} to={to} />;
  ...
  return (
    <div className="gap-y-xlarge flex flex-col">
      <div className="gap-y-large flex flex-col">
        ...
        <div className="medium:grid-cols-2 gap-y-xsmall grid grid-cols-1 gap-x-4">{settings.map((s) => renderCard(s))}</div>
      </div>
