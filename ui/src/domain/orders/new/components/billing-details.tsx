  ...
  return (
    <div className="min-h-[705px]">
      ...
      {!isNullishObject(shippingAddress) || shippingAddressId ? (
        <div className="mt-4 mb-6 flex cursor-pointer items-center" onClick={updateSameAsShipping}>
          <div className={`text-grey-0 border-grey-30 rounded-base flex h-5 w-5 justify-center border ${sameAsShipping && "bg-orange-60"}`}>
      ...
      <div className={clsx({ "pointer-events-none opacity-50": sameAsShipping })} tabIndex={sameAsShipping ? -1 : undefined}>
        <AddressForm countryOptions={validCountries} form={nestedForm(form, "billing_address")} type={AddressType.BILLING} />
      </div>
    </div>
  );
};
