...
import { useAdminCancelOrder, useAdminCapturePayment, useAdminOrder, useAdminRegion, useAdminReservations, useAdminUpdateOrder } from "medusa-react";
...
const OrderDetails = () => {
  ...
  const anyItemsToFulfil = order.items.some((item: LineItem) => item.quantity > (item.fulfilled_quantity ?? 0));
          ...
          <>
            <div>
              {getWidgets("order.details.before").map((widget, i) => {
                return <WidgetContainer key={i} injectionZone={"order.details.before"} widget={widget} entity={order} />;
              })}
            </div>
            <div className="flex space-x-4">
              <div className="gap-y-base flex h-full w-7/12 flex-col">
                ...
                >
                  <div className="mt-6 flex space-x-6 divide-x">
                    <div className="flex flex-col">
                      ...
                      <button className="text-grey-90 active:text-orange-90 flex cursor-pointer items-center gap-x-1" onClick={handleCopyEmail}>
                        {order.email}
                ...
                <BodyCard
                  ...
                  customActionable={<PaymentActionables order={order} capturePayment={capturePayment} showRefundMenu={() => setShowRefund(true)} />}
                >
                  <div className="mt-6">
                        ...
                        {!!payment.amount_refunded && (
                          <div className="mt-4 flex justify-between">
                            <div className="flex">
                              ...
                              <div className="inter-small-regular text-grey-90">{t("details-refunded", "Refunded")}</div>
                            </div>
                            <div className="flex">
                  ...
                  <div className="mt-6">
                    {order.shipping_methods.map((method) => (
                      <div className="flex flex-col" key={method.id}>
                        <span className="inter-small-regular text-grey-50">{t("details-shipping-method", "Shipping Method")}</span>
                    ...
                    <div className="inter-small-regular mt-6 ">
                      {allFulfillments.map((fulfillmentObj, i) => (
                        <FormattedFulfillment key={i} order={order} fulfillmentObj={fulfillmentObj} setFullfilmentToShip={setFullfilmentToShip} />
                      ))}
                    </div>
                  </div>
                </BodyCard>
                <BodyCard className={"h-auto min-h-0 w-full"} title={t("details-customer", "Customer")} actionables={customerActionables}>
                  <div className="mt-6">
                      ...
                      <div>
                        ...
                        {order.shipping_address && (
                          <span className="inter-small-regular text-grey-50">
                            {order.shipping_address.city}, {isoAlpha2Countries[order.shipping_address.country_code?.toUpperCase()]}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div>
                  {getWidgets("order.details.after").map((widget, i) => {
                    return <WidgetContainer key={i} injectionZone={"order.details.after"} widget={widget} entity={order} />;
                  })}
                </div>
            ...
            {emailModal && <EmailModal handleClose={() => setEmailModal(null)} email={emailModal.email} orderId={order.id} />}
            {showFulfillment && (
            ...
            {fullfilmentToShip && (
              <MarkShippedModal handleCancel={() => setFullfilmentToShip(null)} fulfillment={fullfilmentToShip} orderId={order.id} />
            )}
            <OrderEditContext.Consumer>{({ isModalVisible }) => isModalVisible && <OrderEditModal order={order} />}</OrderEditContext.Consumer>
          </>
        )}
      </OrderEditProvider>
    </div>
