...
const SteppedModal: React.FC<SteppedProps> = ({
  ...
  return (
    <ModalElement layeredContext={layeredContext} isLargeModal={isLargeModal} handleClose={resetAndClose}>
      <Modal.Body className={clsx("flex max-h-full flex-col justify-between transition-transform duration-100")}>
            ...
            {!lastScreenIsSummary ||
              (lastScreenIsSummary && context.currentStep !== steps.length - 1 && (
                <div className="flex items-center">
                  <span className="text-grey-50 inter-small-regular mr-4 w-[70px]">{`Step ${context.currentStep + 1} of ${steps.length}`}</span>
                  {steps.map((_, i) => (
                    <span
                      key={i}
                      className={clsx(
                        ...
                        {
                          "outline-orange-20 outline outline-4": context.currentStep === i,
                        }
                      )}
                    />
                  ))}
      ...
      <Modal.Footer>
        <div className="gap-x-xsmall flex w-full justify-end">
          <Button variant="ghost" size="small" disabled={context.currentStep === 0} onClick={() => context.goToPreviousPage()} className="w-[112px]">
            Back
          </Button>
          <Button
            variant="primary"
