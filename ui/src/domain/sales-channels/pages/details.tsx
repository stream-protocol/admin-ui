...
import { useAdminDeleteSalesChannel, useAdminSalesChannels, useAdminStore, useAdminUpdateSalesChannel } from "medusa-react";
...
function ListIndicator(props: ListIndicatorProps) {
  ...
  return (
    <div
      className={clsx("rounded-circle flex h-[18px] w-[18px] flex-shrink-0 items-center justify-center border bg-white", {
        "border-orange-60 border-2": isActive,
      })}
    >
      {isActive && <div className="bg-orange-60 rounded-circle h-[10px] w-[10px]" />}
    </div>
  );
}
...
function SalesChannelDetailsHeader(props: SalesChannelDetailsHeaderProps) {
  ...
  const onDelete = async () => {
    const confirmed = await confirmation({
      text: t("pages-confirm-delete-sales-channel", "Are you sure you want to delete this sales channel? The setup you made will be gone forever."),
...
function Details() {
  ...
  return (
    <div>
      <BackButton path="/a/settings" label={t("pages-back-to-settings", "Back to settings")} className="mb-xsmall" />
