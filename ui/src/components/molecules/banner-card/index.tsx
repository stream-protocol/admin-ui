...
const Description: React.FC<BannerCardDescriptionProps> = ({ cta, children }) => {
  return (
    <div>
      <p className="inter-small-regular text-grey-50 line-clamp-2 max-w-[460px]">{children}</p>
      {cta && (
        <button className="text-orange-60 inter-small-semibold mt-base" onClick={cta.onClick}>
          {cta.label}
        </button>
      )}
    </div>
