...
const NoteInput: React.FC<NoteInputProps> = ({ onSubmit }) => {
  ...
  return (
    <form>
      <div className="py-xsmall px-small bg-grey-5 border-grey-20 rounded-rounded flex items-center border" onClick={() => inputRef.current?.focus()}>
        ...
        <button className="text-grey-30 hover:text-orange-60" type="button" onClick={handleSubmit}>
          <SendIcon size={20} />
        </button>
      </div>
    </form>
