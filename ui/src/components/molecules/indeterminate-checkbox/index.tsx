    ...
    if (type === "radio") {
      return (
        <div className="flex h-full items-center">
          <input className={clsx({ "accent-orange-60": checked })} type="radio" checked={checked} ref={innerRef} {...rest} />
        </div>
      );
    }
      ...
      <div className="flex h-full items-center">
        <div
          ...
          className={`rounded-base border-grey-30 text-grey-0 flex h-5 w-5 cursor-pointer justify-center border ${
            rest.disabled ? (checked ? "bg-gray-300" : "") : checked && "bg-orange-60"
          }`}
        >
        ...
        <input type="checkbox" className={clsx("hidden", className)} defaultChecked={checked} ref={innerRef} {...rest} />
      </div>
    );
  }
);
