...
function ProductCategoryListItemDetails(props: ProductCategoryListItemDetailsProps) {
  const { item } = props;
      ...
      <div style={{ marginLeft: props.depth * -8 }} className="flex h-[40px] items-center">
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center">
            {hasChildren && <div className="absolute flex w-[20px] cursor-pointer items-center justify-center">{props.collapseIcon}</div>}
            ...
            <div className="flex w-[64px] items-center justify-center">
              {!item.is_active && (
                <TooltipIcon
                  ...
                  icon={<TagDotIcon size="32" className="cursor-pointer" outerColor="#FFE5E5" />}
                />
              )}
              {item.is_internal && (
                <TooltipIcon
                  content={t("components-category-visibility-is-private", "Category visibility is private")}
                  icon={<EyeOffIcon color="#889096" size={18} className="cursor-pointer" />}
                />
              )}
            </div>
          </div>
            ...
            <Tooltip
              ...
              content={
                <>
                  ...
                  <span className="text-grey-80 font-semibold">{`"${item.name}"`}</span>
                </>
              }
            >
              <Button size="small" variant="ghost" onClick={() => productCategoriesPageContext.createSubCategory(item)}>
