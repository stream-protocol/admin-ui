import { AdminPostOrdersOrderSwapsReq, Order, ProductVariant, ReturnReason, StockLocationDTO } from "@medusajs/medusa";
...
const SwapMenu: React.FC<SwapMenuProps> = ({ order, onDismiss }) => {
  ...
  const handleProductSelect = (variants: SelectProduct[]) => {
    ...
    setItemsToAdd((itemsToAdd) => [
      ...itemsToAdd,
      ...variants.filter((variant) => !existingIds.includes(variant.id)).map((variant) => ({ ...variant, quantity: 1 })),
    ]);
  };
  ...
  const onSubmit = () => {
    ...
    return mutate(data, {
      onSuccess: () => {
        refetch();
        notification(t("swap-success", "Success"), t("swap-successfully-created-exchange", "Successfully created exchange"), "success");
        onDismiss();
      },
      onError: (err) => {
