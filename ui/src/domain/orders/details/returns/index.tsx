...
import { useAdminRequestReturn, useAdminShippingOptions, useAdminStockLocations, useMedusa } from "medusa-react";
...
const ReturnMenu: React.FC<ReturnMenuProps> = ({ order, onDismiss }) => {
  ...
  const locationsHasInventoryLevels = React.useMemo(() => {
    return Object.entries(toReturn)
      .map(([itemId]) => {
        ...
        const hasInventoryLevel = inventoryMap.get(item.variant_id)?.find((l) => l.location_id === selectedLocation?.value);
  ...
  const onSubmit = async () => {
    ...
    return requestReturnOrder
      .mutateAsync(data)
      .then(() => onDismiss())
      .then(() => notification(t("returns-success", "Success"), t("returns-successfully-returned-order", "Successfully returned order"), "success"))
      .catch((error) => notification(t("returns-error", "Error"), getErrorMessage(error), "error"))
      ...
      <Modal.Body>
        ...
        <Modal.Content>
          <div className="mb-7">
            ...
            <RMASelectProductTable order={order} allItems={allItems} toReturn={toReturn} setToReturn={(items) => setToReturn(items)} />
          </div>
          ...
          <div>
            ...
            <p className="inter-base-regular text-grey-50">
              {t("returns-choose-retur,-shipping-method", "Choose which shipping method you want to use for this return.")}
            </p>
            {shippingLoading ? (
            ...
            <div className="mt-10">
              ...
              <div className="inter-base-semibold flex w-full justify-between">
                ...
                <div className="flex items-center">
                  {!refundEdited && (
                    <>
                      <span className="text-grey-40 mr-2 cursor-pointer" onClick={() => setRefundEdited(true)}>
                        <EditIcon size={20} />{" "}
                      </span>
              ...
              {refundEdited && (
                <CurrencyInput.Root className="mt-2" size="small" currentCurrency={order.currency_code} readOnly>
                  <CurrencyInput.Amount label={t("returns-amount", "Amount")} amount={refundAmount} onChange={handleRefundUpdated} />
                </CurrencyInput.Root>
              )}
            </div>
          )}
        ...
        <Modal.Footer>
          <div className="flex w-full justify-between">
            <div className="flex h-full cursor-pointer items-center" onClick={() => setNoNotification(!noNotification)}>
              <div className={`rounded-base border-grey-30 text-grey-0 flex h-5 w-5 justify-center border ${!noNotification && "bg-orange-60"}`}>
