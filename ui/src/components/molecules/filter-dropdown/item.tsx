 ...
 */
const FilterDropdownItem = ({ filterTitle, options, filters, open, setFilter, isLoading, hasMore, hasPrev, onShowNext, onShowPrev }) => {
  ...
  return (
    ...
    >
      <RadixCollapsible.Root className="w-full" open={open} onOpenChange={(open) => setFilter({ filter: filters, open })}>
        ...
        >
          <div className="flex items-center">
            <div className={`text-grey-0 border-grey-30 rounded-base flex h-5 w-5 justify-center border ${open && "bg-orange-60"}`}>
        ...
        <RadixCollapsible.Content className="w-full">
          {hasPrev && (
            <div className="flex py-2 pl-6">
              <button onClick={handlePrev} className="hover:text-orange-60 text-grey-90 font-semibold">
                Back
              </button>
            </div>
          )}
          {isLoading ? (
            ...
            <DateFilter options={options} open={open} setFilter={setFilter} existingDate={filters} filterTitle={filterTitle} />
          ) : (
            options.map((el, i) => {
              let value: string;
              let label: string;
          ...
          {hasMore && (
            <div className="flex py-2 pl-6">
              <button onClick={handleNext} className="hover:text-orange-60 text-grey-90 font-semibold">
                Show more
              </button>
            </div>
          )}
    ...
    <>
      {options.map((item) => (
        <div
          ...
          className={clsx("hover:bg-grey-5 my-1 flex cursor-pointer items-center rounded px-3 py-1.5", {
            "inter-small-semibold": item === selectedItem,
            "inter-small-regular": item !== selectedItem,
          })}
        >
          <div
            className={clsx("mr-2 flex h-4 w-4 items-center justify-center rounded-full", {
              "border-orange-60 border-2": item === selectedItem,
              "border-grey-30 border ": item !== selectedItem,
            })}
          >
            {item === selectedItem && <div className="bg-orange-60 h-2 w-2 rounded-full" />}
          </div>
          {item}
        </div>
      ))}
...
const RightPopover = ({ trigger, children }) => (
  <RadixPopover.Root>
    <RadixPopover.Trigger className="my-1 w-full">{trigger}</RadixPopover.Trigger>
